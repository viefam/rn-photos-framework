...
export function postAssets(
  ...
  const onTotalProgress = assetUpdatePorgress => {
    if (onProgress) {
      ...
      const loadedProgress = Object.keys(totalProgressLoaded).reduce(
        (loaded, uri) => {
  ...
  const prepareAssets = assets => {
    return new Promise((resolve, reject) => {
      ...
      if (assetsWithoutRoesourceMetaData.length) {
        NativeApi.getAssetsResourcesMetadata(
  ...
  return prepareAssets(assets).then(preparedAssets => {
    ...
    return Promise.all(
      preparedAssets.map(createPostableAsset).map(postableAsset => {
...
export function postAsset(
  postableAsset,
  { url, method, headers, onProgress, onComplete, onError }
) {
  return new Promise((resolve, reject) => {
    ...
    xhr.upload.onprogress = evt => {
      if (evt.lengthComputable) {
        ...
        if (
          lastPercentageComplete &&
          percentComplete === lastPercentageComplete
        ) {
          return;
        }
